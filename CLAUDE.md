...
コードの地図と API 仕様は [`docs/ARCHITECTURE.md`](./docs/ARCHITECTURE.md)。Cron はまだ1行も存在しない。
...
- **認証は Cloudflare Access に任せる**（§6.3）。アプリはトークンを持たず、Worker は `CF_Authorization` の JWT を
  ...
  `DEV_NO_AUTH=1` でだけ検証を外す（`wrangler.jsonc` の vars に置かない）。**`DEV_NO_AUTH` はホスト名が
  ローカル（localhost / プライベート IP / `*.local`）のときしか効かない**。事故で本番に置かれても全公開にしないため。
- **セキュリティヘッダは `vite.config.ts` の `securityHeaders` プラグインが build のたびに `_headers` を生成する。**
  手で書かない。CSP の `script-src` ハッシュは `index.html` のインラインスクリプト（テーマの先当て）から
  毎回計算している。ここを手で書き写すと、あのスクリプトを直した瞬間に CSP が黙って効かなくなる。
  一番効いているのは `connect-src 'self'`（記録の持ち出し口を塞ぐ）と、zxing-wasm のための
  `'wasm-unsafe-eval'`（これを落とすと Chrome で店頭のスキャンが死ぬ）。`/api/*` のヘッダは `server/app.ts` が付ける。
- **外部APIから受け取った値は `server/sources/http.ts` の `clean` / `cleanImageUrl` を必ず通す。**
  D1 に焼かれて以後ずっと全端末へ配られる値なので、長さで切り、画像 URL は https 以外を落とす。
...
- **サーバーが上限で断る形のリクエストは、クライアント側で作らない**。`buildChanges` が1往復の合計で切り、
  1行で上限を越える行は諦めて未送信から下ろす。「毎回同じ 500 行を作って毎回断られる」に落ちると、
  設定画面にエラーが残るだけで自力で戻れない（同期は誰も待っていないので、誰も気づかない）。
  **`settle` に渡すのは実際に送った集合（`sent`）で、`dirty` ではない**。取り違えると、
  送っていない行を未送信から消す＝その行が控えに載らないまま消える。
...
- **JSON / CSV 完全エクスポートは最初のリリースに含める**（§6.4）。後付けにすると実装されない。
  CSV のセルは `=` `+` `-` `@` 始まりを `'` で文字列に倒す（Excel / Sheets の数式インジェクション）。
  商品名は EC の出品タイトル由来＝第三者が書いた文字列なので、自分のデータだからと素通しにしない。
